function [ name ] = RagRamPictureGrid( MMML_dataset,used,concentrations_to_plot, t, izmers)
...
%   Plot picture grid with same scale
    ...
    for i=1:numel(concentrations)
        ...
        for j=1:length(concentration_order)
            ...
            for n=1:length(t)
                ...
                xcoordinate = dxs+dxx*(j-1) + dx*(j-1);
                ...
                if i==1
                    ...
                    text(xcoordinate + 0.5*(dxx+dx), 0.05,[sprintf('%s%g-%g ',char(ramString(j)),RAM_INTERVALS(2*j-1),RAM_INTERVALS(2*j))],...
    ...
    name = sprintf('Rag_all_Ram_all_time_%gs_%.1fx%.1fmm_v1',t(1)/1000, bbox_konfiguracija(1), bbox_konfiguracija(1));
end
